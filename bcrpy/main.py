...
class Marco:
    ...
    def parameters(self):
        ...
        decoration = colored(
            "Estado actual de parametros constructores del objeto:", "green"
        )
...
"""
    ...
    def wordsearch(self, keyword="economia", cutoff=0.65, columnas="all"):
        ...
        print(
            "; columnas =",
            "`" + columnas + "`(todas)" if columnas == "all" else columnas,
        )
    ...
    def order_columns(self, hacer=True):
        ...
        user_order = [
            "{} - {}".format(
                self.query_dict(codigo)["Grupo de serie"],
                self.query_dict(codigo)["Nombre de serie"],
            )
    ...
    def GET(self, forget=False, order=True, datetime=True):
        ...
        if os.path.exists(cache_filename) and not forget:
            print(
                colored(
                    "Obteniendo información de datos desde la memoria caché",
                    "green",
                    attrs=["blink"],
                )
            )
            ...
            print(
                colored(
                    "Obteniendo información con la URL de arriba usando requests.get. Por favor espere...",
                    "green",
                    attrs=["blink"],
                )
            )
            ...
            df = pandas.DataFrame(
                columns=header
            )  # aqui todavia no se han insertado los datos en las filas

            for j in dict["periods"]:
                # print(j['values'])
                df.loc[j["name"]] = [
                    float(ij) if ij != "n.d." else None for ij in j["values"]
                ]

            if datetime:
                df.index = pandas.to_datetime(
                    df.index
                )  # convierte fechas a datetime (para facilitar visual acercar y alejar ticks de las graficas)
    ...
    def get_data_for_chunk(self, chunk):
        ...
        df.columns = [
            f"{col}, codigo no. {chunk[idx]}" for idx, col in enumerate(df.columns)
        ]
    ...
    def largeGET(self, codigos=[], chunk_size=100, turbo=True, nucleos=4):
        ...
        codigo_chunks = [
            codigos[i : i + chunk_size] for i in range(0, len(codigos), chunk_size)
        ]
        ...
        if turbo:
            ...
            for idx, chunk in enumerate(codigo_chunks):
                try:
                    ...
                    all_chunks.append(data_chunk)
                    print(
                        f"Fragmento {idx + 1}/{len(codigo_chunks)} obtenido exitosamente."
                    )
                except Exception as e:
        ...
        self.codigos = (
            [i.split(", codigo no. ")[-1] for i in final_dataframe.columns]
            if turbo
            else codigos
        )
        ...
        print(
            "Lista de codigos en su orden en el dataframe : self.codigos (arriba ^^)\n"
        )
